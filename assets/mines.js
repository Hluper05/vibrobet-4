...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-aec0efef";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-aec0efef">'+'<span class="tile-face-aec0efef tile-front-aec0efef"></span>'+'<span class="tile-face-aec0efef tile-back-aec0efef"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-aec0efef")){return;}
var back=tile.querySelector(".tile-back-aec0efef");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-aec0efef");if(isMine){tile.classList.add("boom-aec0efef");}
if(dim){tile.classList.add("dimmed-aec0efef");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-aec0efef")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-aec0efef");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-aec0efef")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-aec0efef");void panelEl.offsetWidth;panelEl.classList.add("jolt-aec0efef");}
endRound();}
